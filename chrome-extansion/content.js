    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        ...
        if (introContainer) {
            ...
            for (let i = h1Index + 2; i < children.length; i++) {
                ...
                if (text && text.length < 120) { // location lines are short
        ...
        if (!location) {
            location =
                document.querySelector(".not-first-middot span[aria-hidden='true']")?.innerText?.trim() ||
                document.querySelector("span.t-black--light.t-normal")?.innerText?.trim()?.split("·")[0]?.trim() ||
                null;
        }
        ...
        if (introContainer) {
            console.log("[Bitzsol CRM] Intro children:",
                [...introContainer.children].map((el, i) => `[${i}] ${el.tagName}: "${el.innerText?.trim()?.slice(0, 60)}"`));
        }
